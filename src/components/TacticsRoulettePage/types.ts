...
export const hostageMaps: readonly MapName[] = ["Office", "Italy", "Agency"] as const;
...
export function isTacticValidForMap(tacticMapContext: TacticMapContext, targetMap: MapName): boolean {
	// If tactic is for a specific map, it must match exactly
	if (hostageMaps.includes(tacticMapContext as MapName) || defuseMaps.includes(tacticMapContext as MapName)) {
...
export function getValidMapsForTactic(tacticMapContext: TacticMapContext): readonly MapName[] {
...
export function filterTacticsForMap<T extends Tactic>(tactics: readonly T[], selectedMap: MapName): readonly T[] {
	return tactics.filter(tactic => isTacticValidForMap(tactic.map, selectedMap));
}
