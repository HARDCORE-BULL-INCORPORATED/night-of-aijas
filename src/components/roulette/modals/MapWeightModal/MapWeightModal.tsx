...
const MapWeightModal: Component<MapWeightModalProps> = (props) => {
    ...
    createEffect(() => {
        if (props.isOpen) {
            setEditableMapConfigs(
                props.currentMapConfigs.map((map) => ({ ...map })),
            );
    ...
    const totalWeight = createMemo(() => {
        return editableMapConfigs.reduce(
            (sum, map) => sum + (map.weight || 0),
            0,
        );
    });
            ...
            >
                ...
                >
                    ...
                    <p class={styles.explanationText}>
                        Adjust the weights to change the likelihood of each map
                        being selected. A higher weight means a higher chance.
                        For example, a map with weight 2 is twice as likely to
                        be picked as a map with weight 1. Weights are relative
                        to each other.
                    </p>
                    ...
                    >
                        ⚠️ Note: Updating the map pool (adding/removing maps)
                        will reset all weights to 1 for now.
                    </p>
                                ...
                                {(map) => {
                                    ...
                                    return (
                                        <li class={styles.mapWeightItem}>
                                            ...
                                            />
                                            <span class={styles.mapName}>
                                                {map.name}
                                            </span>
                                            <input
                                                type="text"
                                                value={
                                                    weightInputs[map.name] ||
                                                    map.weight.toString()
                                                }
                                                onInput={(e) =>
                                                    handleWeightChange(
